            ...
            <div className="flex w-full items-center justify-between px-4">
              <div className="flex items-center gap-8">
                ...
                <div>
                  ...
                  >
                    <span
                      className={`relative my-1.5 block h-0.5 w-[30px] bg-lime-200 transition-all duration-300 ${
                      ...
                      {menuData.map((menuItem, index) => (
                        <li key={menuItem.id} className="group relative">
                          {menuItem.path ? (
                            <Link
                              href={menuItem.path}
                              className={`flex py-2 text-slate-500 group-hover:opacity-70 lg:mr-0 lg:inline-flex lg:py-6 lg:px-0`}
                            >
                              {menuItem.title}
                            </Link>
                          ) : (
                            <>
                              <a
                                ...
                                className="flex cursor-pointer items-center justify-between py-2 text-base text-slate-500 group-hover:opacity-70 lg:mr-0 lg:inline-flex lg:py-6 lg:px-0"
                              >
                                {menuItem.title}
                                <span className="pl-3">
                                  <svg
                              ...
                              >
                                {menuItem.submenu?.map((submenuItem) => (
                                  <Link
                                    ...
                                    key={submenuItem.id}
                                    className="block rounded py-2.5 text-sm text-slate-500 hover:opacity-70 lg:px-3"
                                  >
                                    {submenuItem.title}
                                  </Link>
                                ))}
              ...
              <div className="flex items-center justify-end pr-16 lg:pr-0">
                <Link
                  href="/signin"
                  className="hidden py-3 px-7 text-base font-bold text-slate-500 hover:opacity-70 md:block"
                >
                  Sign In
                </Link>
                <Link
                  href="/signup"
                  className="ease-in-up hidden rounded-md bg-primary py-3 px-8 text-base font-bold text-slate-500 transition duration-300 hover:bg-opacity-90 hover:shadow-signUp md:block md:px-9 lg:px-6 xl:px-9"
                >
                  Sign Up
                </Link>
              </div>
